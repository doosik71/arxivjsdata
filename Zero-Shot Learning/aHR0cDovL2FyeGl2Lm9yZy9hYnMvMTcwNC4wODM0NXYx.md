...
1. **선형 오토인코더 기반:** 입력 데이터 $X \in \mathbb{R}^{d \times N}$ (N개의 $d$차원 특징 벡터)를 저차원 잠재 공간 $S \in \mathbb{R}^{k \times N}$으로 투영하는 인코더 $W \in \mathbb{R}^{k \times d}$와 $S$를 다시 $X$로 재구성하는 디코더 $W^* \in \mathbb{R}^{d \times k}$를 정의한다. 목표는 재구성 오차 $ \min_{W, W^*} \|X - W^*WX\|_F^2 $를 최소화하는 것이다.
