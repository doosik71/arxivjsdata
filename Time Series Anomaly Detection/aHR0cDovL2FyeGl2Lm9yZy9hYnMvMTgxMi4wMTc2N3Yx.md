...
1. **노이즈 제거 (Noise Removal):**
   ...
   - $$ y'_{t} = \sum_{j \in J} w_{tj} y_j $$
   - 필터 가중치 $w_{tj}$는 시간적 거리와 값의 유사성을 모두 고려한 두 가우시안 함수의 곱으로 정의됩니다.
   - $$ w_{tj} = \frac{1}{z} e^{-\frac{|j-t|^2}{2\delta^2_d}} e^{-\frac{|y_j-y_t|^2}{2\delta^2_i}} $$
...
2. **추세 추출 (Trend Extraction):**
   ...
   - $$ \min_{\nabla\tau} \sum_{t=T+1}^N |g*t - \sum_{i=0}^{T-1} \nabla\tau_{t-i}| + \lambda_1 \sum_{t=2}^N |\nabla\tau*t| + \lambda_2 \sum_{t=3}^N |\nabla^2\tau_t| $$
