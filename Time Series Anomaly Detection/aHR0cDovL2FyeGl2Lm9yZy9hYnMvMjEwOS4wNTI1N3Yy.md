...
1. **PA 프로토콜의 과대평가 분석:**
   - 표준 F1 점수($F1 = \frac{2 \cdot P \cdot R}{P + R}$) 계산을 정의하고, PA가 TP(True Positives)를 증가시키고 FN(False Negatives)을 감소시켜 F1 점수를 인위적으로 높이는 방식을 설명합니다.
     $$\hat{y}_{t} = \begin{cases} 1, & \text{if } A(w_{t}) > \delta \text{ or } t \in S_{m} \text{ and } \exists t' \in S_{m} A(w_{t'}) > \delta \\ 0, & \text{otherwise} \end{cases}$$
