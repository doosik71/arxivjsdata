...
1. **잠재 변수 및 상호 정보량 최대화**:
   ...
   - InfoGAIL의 목적 함수는 다음과 같습니다:
     $$ \min_{\pi,Q} \max_{D} E_{\pi}[\log D(s,a)] + E_{\pi_E}[\log(1-D(s,a))] - \lambda_1 L_I(\pi,Q) - \lambda_2 H(\pi) $$
...
2. **보상 증강(Reward Augmentation)**:
   ...
   - 확장된 목적 함수는 다음과 같습니다:
     $$ \min_{\theta,\psi} \max_{\omega} E_{\pi*\theta}[\log D_\omega(s,a)] + E_{\pi_E}[\log(1-D*\omega(s,a))] - \lambda*0 \eta(\pi*\theta) - \lambda*1 L_I(\pi*\theta,Q*\psi) - \lambda_2 H(\pi*\theta) $$
...
3. **최적화 개선**:
   ...
   - **Wasserstein GAN (WGAN)** [26] 사용: 기존 GAN 목적 함수의 기울기 소실 및 모드 붕괴 문제를 완화하여 복잡한 다중 모드 궤적 분포 모델링에 유리합니다.
     $$ \min_{\theta,\psi} \max_{\omega} E_{\pi*\theta}[D_\omega(s,a)] - E_{\pi_E}[D*\omega(s,a)] - \lambda*0 \eta(\pi*\theta) - \lambda*1 L_I(\pi*\theta,Q*\psi) - \lambda_2 H(\pi*\theta) $$
