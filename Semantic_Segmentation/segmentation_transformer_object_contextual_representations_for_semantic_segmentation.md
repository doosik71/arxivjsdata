...
- **3단계 OCR 프레임워크:**
  1. 정답(ground-truth) 분할의 감독 하에 소프트 객체 영역(soft object regions)을 학습합니다.
...
1. **소프트 객체 영역 형성 (Soft Object Regions):**
   ...
   - 이 객체 영역들은 백본 네트워크(예: ResNet 또는 HRNet)에서 출력된 중간 표현으로부터 계산되며, 훈련 중 정답 분할(ground-truth segmentation)을 사용하여 교차 엔트로피 손실(cross-entropy loss)로 학습됩니다.
...
- **영역 감독의 중요성:** 소프트 객체 영역 형성 과정에 대한 지도 감독(supervision)이 분할 성능에 결정적인 역할을 한다는 것을 경험적으로 보여주었습니다. 이는 의미론적 정보가 컨텍스트 집약 과정에 통합될 때 더 정확한 관계 학습이 가능함을 의미합니다.
