...
1. **조건부 도메인 판별자 ($D(T(h))$)**:
   ...
   - **최적화 목표**:
     $$ \min_{G} E_{(x*s,y_s) \sim \mathcal{D}_s} L(G(x_s),y_s) + \lambda \left( E_{x*s \sim \mathcal{D}_s} \log [D(T(h_s))] + E_{x*t \sim \mathcal{D}_t} \log [1-D(T(h_t))] \right) $$
        $$ \max_D E_{x*s \sim \mathcal{D}_s} \log [D(T(h_s))] + E_{x_t \sim \mathcal{D}_t} \log [1-D(T(h_t))] $$
...
4. **엔트로피 조건화 (CDAN+E)**:
   ...
   - **최적화 목표 (CDAN+E)**:
     $$ \min_{G} E_{(x*s,y_s) \sim \mathcal{D}_s} L(G(x_s),y_s) + \lambda \left( E_{x*s \sim \mathcal{D}_s} w(H(g_s)) \log [D(T(h_s))] + E_{x*t \sim \mathcal{D}_t} w(H(g_t)) \log [1-D(T(h_t))] \right) $$
        $$ \max_D E_{x*s \sim \mathcal{D}_s} w(H(g_s)) \log [D(T(h_s))] + E_{x_t \sim \mathcal{D}_t} w(H(g_t)) \log [1-D(T(h_t))] $$
