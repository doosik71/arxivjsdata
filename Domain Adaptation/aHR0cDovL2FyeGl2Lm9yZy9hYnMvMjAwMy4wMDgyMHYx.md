...
**전체 학습 목표 함수:**
$$ L_{\text{MADAN}} = \sum_{i=1}^{M} \left[ L_{S_i \to T}^{\text{GAN}} + L_{T \to S_i}^{\text{GAN}} + L_{S_i \leftrightarrow T}^{\text{cyc}} + L_{S_i}^{\text{DSC}} + L_{S_i}^{\text{SAD}} + L_{S_i}^{\text{CCD}} \right] + L_{\text{task}} + L^{\text{FLA}} $$
$$ L_{\text{MADAN+}} = L^{\text{CAG}} + \sum_{i=1}^{M} \left[ L_{S*i}^{\text{SAD}} + L_{S*i}^{\text{CCD}} \right] + L_{\text{task}} + L^{\text{FLA}} + L^{\text{CLA}} $$
