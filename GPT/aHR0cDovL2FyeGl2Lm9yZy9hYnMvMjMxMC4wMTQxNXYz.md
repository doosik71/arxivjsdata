...
2. **프롬프트-추론-미세조정(Prompting-Reasoning-Finetuning) 전략:**
    ...
    * **미세조정(Fine-tuning):** OpenAI 미세조정 API를 사용하여 LLM의 출력 $\{T, R\}$을 인간 주행 궤적 $\hat{T}$ 및 자동으로 생성된 정답 추론 $\hat{R}$에 맞춰 조정합니다. 추론 과정 $\hat{R}$은 가상의 자차 궤적과 주변 객체들의 겹침 여부를 분석하여 자동으로 생성됩니다.
