...
1. **베이스라인 모델 (FCN-8s 확장)**:
   ...
   - **FCN-8s-F2**: 이전 블록들로부터 더 많은 스킵($N=3$)을 추가합니다.
     $$ y*L = F(x_L) + W_L x_L + W_{L-1} x_{L-1} + W_{L-2} x_{L-2} $$
...
2. **마스터-슬레이브 네트워크 (MSNet) 설계**:
   ...
   - **후방향 스킵 연결 공식**: 마스터 네트워크의 블록 $L$에 $P$개의 후방향 스킵 연결을 추가합니다.
     $$ y_{m_L} = F(x_{m*L} + \sum_{l=L}^{L+(P-1)} U*l y_{s*l}) $$
     여기서 $y_{m*L}$은 마스터 네트워크의 블록 $L$의 출력, $y_{s_l}$은 슬레이브 네트워크의 블록 $l$의 출력, $U_l$은 슬레이브 네트워크의 특징 맵에 적용되는 합성곱 변환입니다.
   - **통합 모델 (MSNet-FB1/FB2)**: 마스터 네트워크에 전방향 스킵 연결(FCN-8s-F1 또는 FCN-8s-F2)과 후방향 스킵 연결을 모두 통합합니다.
     $$ y_{m_L} = F(x_{m*L} + \sum_{l=L}^{L+(P-1)} U*l y_{s*l}) + \sum_{l=L-(N-1)}^{L} W*l x_{m_l} $$
