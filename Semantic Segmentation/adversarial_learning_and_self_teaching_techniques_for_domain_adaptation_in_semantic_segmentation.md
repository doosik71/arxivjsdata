...
1. **지도 학습 손실 ($L_{G,1}$):**
   ...
   - $$ L_{G,1} = - \sum_{p \in X*s^n} \sum_{c \in C} Y*s^n(p)_{[c]} \cdot \log(G(X*s^n)(p)_{[c]}) $$
...
3. **자가 학습 손실 ($L_{G,3}$):**
   ...
   - $$ L_{G,3} = - \sum_{p \in X*t^n} \sum_{c \in C} D*R(X_t^n)(p) \cdot W_s^c \cdot \hat{Y}_n(p)_{[c]} \cdot \log(G(X*t^n)(p)_{[c]}) $$
