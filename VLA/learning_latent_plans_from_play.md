...
- **18가지 시각 조작 작업에서의 탁월한 성능:**
  - Play-LMP는 작업 라벨 없이 훈련되었음에도 불구하고, 픽셀 기반 실험에서 69.4%, 정답 상태(ground truth state) 기반 실험에서 85.5%의 성공률을 달성했습니다. 이는 18가지 개별 전문가 훈련 BC(Behavioral Cloning) 정책(픽셀 66.5%, 상태 70.3%) 및 다중 작업 BC 정책(상태 66.2%)을 크게 능가하는 결과입니다.
