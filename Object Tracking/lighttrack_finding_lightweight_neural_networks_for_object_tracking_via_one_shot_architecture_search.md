...
1. **백본 슈퍼넷(Backbone Supernet) 사전 학습**:
   ...
   - 이 슈퍼넷은 ImageNet 데이터셋을 사용하여 이미지 분류 손실($L_{\text{cls_pre-train}}$)로 한 번만 사전 학습됩니다.
...
2. **추적 슈퍼넷(Tracking Supernet) 학습**:
   ...
   - 이 슈퍼넷은 Youtube-BB, ImageNet VID/DET, COCO, GOT-10K 등 추적 데이터셋을 사용하여 추적 손실($L_{\text{trk_train}}$)로 공동으로 학습됩니다.
