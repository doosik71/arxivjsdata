...
- **언어 모델(Language Model)**:
  - 외부 텍스트 코퍼스로 훈련된 n-gram 언어 모델(영어: 5-gram Kneser-Ney, 만다린어: 문자 수준 5-gram Kneser-Ney)을 디코딩 시 빔 서치에 통합합니다($Q(y) = \log(p_{\text{ctc}}(y|x)) + \alpha\log(p_{\text{lm}}(y)) + \beta\text{word_count}(y)$).
